...
const config = {
    user: 'ld',
    password: 'ld',
    server: '192.76.1.20', // You can use 'localhost\\instance' to connect to named instance
    database: 'ERP4',
    options: {
...
function reqCallback(err, response, body) {
    if (!err && response.statusCode == 200) {
        ...
        for (i; i < len; i++) {
            ...
            dbhelper.add({publishdate:timestr,price:price,type:1,source:"中国银行"},"[ERP4].[dbo].[外销电商_爬虫数据USDCNY]",function (err) {
                console.log(err);
            })
